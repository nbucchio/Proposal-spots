...
//                           https://www.proposalspots.com/api/pinterest/callback
...
//   PINTEREST_MODE          'sandbox' (default, for Trial access) or 'production'
//                           (only once Standard access is approved)
//
...
// The consent screen (where the user clicks "Give access") is always on
// www.pinterest.com, in both sandbox and production.
const OAUTH_BASE = 'https://www.pinterest.com';

// Which Pinterest API environment to talk to. THIS MATTERS:
//   - Trial access apps MUST use the SANDBOX (api-sandbox.pinterest.com).
//     Production rejects them with: "Apps with Trial access may not create Pins
//     in production ... use API Sandbox instead." Sandbox pins/boards are
//     private test data — exactly what the Standard-access demo video needs.
//   - Standard access apps use PRODUCTION (api.pinterest.com) to publish real,
//     public pins.
//
// Controlled by the PINTEREST_MODE env var. Defaults to 'sandbox' because the
// app starts on Trial access. Once Pinterest approves Standard access, set
// PINTEREST_MODE=production in Vercel and re-run the OAuth connect flow to mint
// a production token.
//
// NOTE: the access/refresh tokens are environment-specific. A token minted
// against the sandbox only works on the sandbox, and vice-versa — so whenever
// you switch PINTEREST_MODE you must reconnect to get a matching token.
function apiBase() {
  const mode = (process.env.PINTEREST_MODE || 'sandbox').toLowerCase();
...
export function getRedirectUri(req) {
  ...
  // Sensible production default (www is the canonical Proposal Spots domain).
  return 'https://www.proposalspots.com/api/pinterest/callback';
}
...
async function pinterestFetch(path, options = {}, accessToken) {
  ...
  const doFetch = (bearer) => fetch(`${apiBase()}${path}`, {
    ...options,
    headers: {
